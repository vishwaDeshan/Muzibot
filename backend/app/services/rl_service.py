...
class RLRecommendationAgent:
    ...
    def _save_q_table_for_song(self, song_id: str, q_table: np.ndarray, arousal_idx: int, valence_idx: int, exact_arousal: float, exact_valence: float):
        try:
            ...
            self.db.rollback()
            logging.exception(f"Failed to save Q-table for song {song_id}: {str(e)}")
            raise
    ...
    def get_optimized_weights(self, song_id: str, mood: str, prev_rating: int):
        ...
        if np.max(slice) == 0:
            return {
                ...
                'desired_mood_after_listening': self.weight_values[2]
            }
        w_similar_idx, w_current_idx, w_desired_idx = np.unravel_index(np.argmax(slice), slice.shape)
        return {
    ...
    def _save_weights(self, mood: str, weights: dict):
        ...
        try:
            ...
            self.db.rollback()
            logging.exception(f"Failed to save weights for mood {mood}: {str(e)}")
            raise
